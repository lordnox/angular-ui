  ...
  Templates = {
    header:   '<thead>' +
                '<th ng-repeat="head in ngGrid.header">' +
                  '<a class="span3 btn" ng-class="{disabled: !head.name}"  ng-click="sort(head.index)">' +
    ...
    controls: '<div class="row">' +
                '<div class="span4">' +
                  '<select ng-options="option for option in ngGrid.limits" ng-model="ngGrid.limit"></select>' +
                '</div>' +
                '<div class="control-group span8" ng-class="{error: ngGrid.filterError}">' +
                  '<input type="text" style="float:right;" class="input-medium" ng-model="ngGrid.filter" placeholder="{{i18n.filter}}" name="filter" />' +
                '</div>' +
              '</div>',
    footer:   '<div style="float: left;" class="pageNum">' +
                '<strong>{{i18n.total}}{{ngGrid.filteredList(true).length}} [{{ngGrid.unfilteredList(true).length}}]</strong>' +
              '</div>' +
  ...
  ngGridModule.directive('ngGrid',  function($compile) {
    return {
      compile: function compile(element) {
        ...
        var templates = {
          header  : $('template#ngGrid-grid-header').html() || Templates.header
        , controls: $('template#ngGrid-grid-controls').html() || Templates.controls
        , footer  : $('template#ngGrid-grid-footer').html() || Templates.footer
        , debug   : $('template#ngGrid-grid-debug').html() || Templates.debug
        };
